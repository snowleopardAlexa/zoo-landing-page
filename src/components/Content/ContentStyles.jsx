...
export const Heading = styled(motion.h2)`
	...
	line-height: 1.1;
	font-weight: 600;
	color: ${({ inverse }) => (inverse ? '#EAEAEA' : '#e65c00')};
...
export const ContentButton = styled(motion.button)`
	...
	cursor: pointer;
	background: none;
	color: ${({ inverse }) => (inverse ? 'white' : '#0f9b0f')};
	white-space: nowrap;
	padding: ${({ big }) => (big ? '12px 64px' : '10px 20px')};
	font-size: ${({ fontBig }) => (fontBig ? '20px' : '16px')};
	outline: none;
	border: 2px solid ${({ inverse }) => (inverse ? 'white' : '#061700')};
	cursor: pointer;
	position: relative;
	overflow: hidden;
	...
	&:hover {
		background: ${({ inverse }) => (inverse ? '#61045F' : '#0f9b0f')};
